...
class MM extends REST_Controller
{
	...
	public function getDataSimpanan_get()
	{
		$kode = $this->input->get("nik");
		$query = $this->db->query("
						SELECT *
						FROM anggota
						WHERE nik = '" . $kode . "'
		...
		if ($query->num_rows() > 0) {
			...
			$total = $row->simpanan_pokok
				+ $row->total_simpanan_anggota
				+ $row->simpanan_khusus
				+ $row->total_SHU
				+ $row->total_tabungan
				+ $row->total_cadangan
				+ $row->total_tabungan_sisuka
				+ $row->total_jasa_sisuka
				+ $row->total_bunga_swt;
			...
			$dt = [			
				'tvTotalSaldo'   => $total,
				'tvSPokok'       => $row->simpanan_pokok,
				'tvSWajib'       => $row->total_simpanan_anggota,
				'tvSKhusus'      => $row->simpanan_khusus,
				'tvSHU'          => $row->total_SHU,
				'tvTabungan'     => $row->total_tabungan,
				'tvCadangan'     => $row->total_cadangan,
				'tvSisuka'       => $row->total_tabungan_sisuka + $row->total_jasa_sisuka,
				'tvSwTransaksi'  => $row->total_bunga_swt,
			];
