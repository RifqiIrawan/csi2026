...
class Visiting extends CI_Controller {
	...
	public function index($submenu = null, $id = null){  
        ...
        switch (strtolower($submenu)) {
            ...
            case "why-visit-section-datatable":
                ...
                break;
            case "why-visit-section-add":
                $this->why_visit_section_add();
                break;
            ...
            case "conference-banner-add":
                ...
                break;
            case "conference-banner-update":
                $this->conference_banner_update();
                break;
  ...
  public function why_visit_banner_add(){
    ...
    $config['upload_path'] = $file_path; // FCPATH = path ke public root CI
    ...
    if (!empty($_FILES['banner_whyvisitimage']['name'])) {
        if ($this->upload->do_upload('banner_whyvisitimage')) {
  ...
  public function why_visit_banner_update_good()
  {
    ...
    public function why_visit_banner_delete($id = null)
    {
        ...
        try {
            ...
            // ===== Ambil data media ===== //
            $media = $this->db->get_where('csi_content_media', ['content_id' => $id])->row();
    ...
    public function conference_banner_add(){
        ...
        // print_r($_FILES);
        // die();
        $addsectionyear        = trim($this->input->post('addsectionyear'));
        $addsectiontitle       = trim($this->input->post('addsectiontitle'));
        $addsectionStatus      = trim($this->input->post('addsectionStatus'));

        // Validasi minimal
        if (empty($addsectionyear)) {
            echo json_encode([
                'success' => false,
                'message' => 'Banner year dan title wajib diisi.'
            ]);
        ...
        try {
            ...
            $data = [
                'menu_id'       => $menu_id,
                'content_year'  => $addsectionyear,
                'content_type'  => $bannertype,
                'title'         => $addsectiontitle,
                'status'        => $addsectionStatus,
                'created_date'  => $created_date,
                'created_by'    => $created_by,
                'modified_date' => $created_date,
                'modified_by'   => $created_by
    ...
    public function conference_banner_update(){
        ...
        // ===== Ambil Data Lama ===== //
        $oldMedia = $this->db->get_where('csi_content_media', ['id' => $mediaid])->row();
        $oldImage = !empty($oldMedia) ? basename($oldMedia->file_path) : null;
    ...
    public function why_visit_section_add()
    {
            ...
            try {
                ...
                if ($image) {
                    $datasectionMedia = [
                        'content_id'      => $content_id,     // the related content ID
                        'media_type'      => 'image',     // e.g., 'image', 'video', etc.
                        'file_path'       => $file_path_save . $image,      // path on server
                        'sort_order'      => $sort_order,     // integer
                        'is_main'         => $is_main,        // 0 or 1
                        'created_date'    => $created_date,   // usually date('Y-m-d H:i:s')
                        'created_by'      => $created_by,     // user id or name
    ...
    public function why_visit_section_delete($id = null)
    {
        $id       = (int) $id;
        $id_menus = 10;
        ...
        try {
            ...
            if (!$deleted) {
                throw new Exception('Gagal menghapus content');
            }
